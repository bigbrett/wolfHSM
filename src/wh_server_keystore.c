...
#ifdef WOLFHSM_CFG_GLOBAL_KEYS
/**
 * @brief Build server keyId from client request
 *
 ...
 * @param _clientId Client ID from server->comm->client_id
 * @param _reqId Client's requested ID (may include WH_KEYID_GLOBAL flag)
 */
#define WH_SERVER_BUILD_KEYID(_type, _clientId, _reqId)                        \
                          ...
                          uint16_t wrappedKeySz)
{
    ...
    ret = wh_Server_KeystoreReadKey(
        server,
        WH_SERVER_BUILD_KEYID(WH_KEYTYPE_CRYPTO, server->comm->client_id,
                              serverKeyId),
                                ...
                                uint8_t* respData, uint32_t respDataSz)
{
    ...
    if (!WH_KEYID_ISERASED(metadata.id)) {
        ...
        metadata.id = WH_SERVER_BUILD_KEYID(
                               ...
                               void* resp_packet)
{
    ...
    switch (action) {
        case WH_KEY_CACHE: {
            ...
            meta->id     = WH_SERVER_BUILD_KEYID(WH_KEYTYPE_CRYPTO,
        ...
        case WH_KEY_EVICT: {
            ...
            ret = wh_Server_KeystoreEvictKey(
                server, WH_SERVER_BUILD_KEYID(WH_KEYTYPE_CRYPTO,
                                              server->comm->client_id, req.id));
            resp.rc = ret;
            /* TODO: Are there any fatal server errors? */
            ret = WH_ERROR_OK;
