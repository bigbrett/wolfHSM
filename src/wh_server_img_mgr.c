                                       ...
                                       uint16_t*       sig_sz_out)
{
    ...
    memcpy(&img_size, hdr + 4, sizeof(img_size));
    ...
    const uint8_t* key, size_t keySz, const uint8_t* sig, size_t sigSz)
{
    ...
    /* DMA pre-process payload */
    ret = wh_Server_DmaProcessClientAddress(
        server, img->addr, &serverPayloadPtr, img->size,
        WH_DMA_OPER_CLIENT_READ_PRE, (whServerDmaFlags){0});
    if (ret != WH_ERROR_OK) {
        goto cleanup;
    }
    ...
    ret = _wolfBootImgValidateAndHash(hdr, img->hdrSize, payload, img->size,
