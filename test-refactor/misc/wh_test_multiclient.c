 ...
 * - Client 1 creates and increments a counter in the global namespace
                              ...
                              whServerContext* server2)
{
    ...
    /* Client 1 creates a global counter starting at 41 */
    ...
    /* Client 2 reads the SAME global counter and sees the shared value */
    ...
    /* Isolation: client 2's own plain counter 7 is a different object that was
     * never created */
...
/*
 * Non-add verbs must reject ids with bits above the id and client-flag
 * fields instead of silently truncating them onto another object. Add object
 * 0x42, then confirm the legacy-style id 0x1042 cannot read, stat, list-from,
 * or destroy it.
 */
                                        ...
                                        whServerContext* server2)
{
    ...
    const whNvmId aliasId = 0x1042; /* truncates to 0x42 if not rejected */
    int32_t       out_rc  = 0;
    whNvmSize     out_len = 0;
    whNvmId       list_id = 0;
    whNvmId       count   = 0;
    ...
    /* The destructive verb is the dangerous one: without the per-verb check,
     * destroying 0x1042 silently destroyed 0x42 */
    out_rc = 0;
    WH_TEST_RETURN_ON_FAIL(
...
#ifdef WOLFHSM_CFG_DMA
    /* The DMA read path shares the check */
    out_rc = 0;
    WH_TEST_RETURN_ON_FAIL(
    ...
    /* The object the malformed id would have truncated onto is untouched */
    out_rc = 0;
    WH_TEST_RETURN_ON_FAIL(_nvmReadViaServer(
                                          ...
                                          whServerContext* server2)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        /* Read: must not return the planted bytes. The flagged id is rejected
         * up front; the translation-time strip is only a backstop. */
        memset(buf, 0, sizeof(buf));
        out_rc = 0;
        WH_TEST_RETURN_ON_FAIL(_nvmReadViaServer(
